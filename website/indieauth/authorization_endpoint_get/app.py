...
def display_confirmation_screen(event, session_key, session):
    ...
    with open("get.html") as template:
        context = params.copy()
        context["csrf_token"] = csrf_token(session_key, session)
        html = chevron.render(template, data=context)
...
def lambda_handler(event, context):
    ...
    if not (
        ...
        return display_confirmation_screen(event, session_key, session)
        print(token)
